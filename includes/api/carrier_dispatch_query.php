<?php 
    ...
    if (isset($_POST['miles'])) {$miles = mysql_prep($_POST["miles"]);} else { $miles = 0;}
    ...
    if (empty($errors)) { 
        ...
        $query = "INSERT INTO dispatch_list (company_id, carrier_id, truck_id, dispatcher_id, dispatch_team_id, current_location, new_location, miles, rate, commission, delivery_time, dispatch_time) ";
        $query .= "VALUES ('" . $carrier["company_id"] . "','" . $carrier["id"] . "', '" . $truck_id ."','" . $dispatcher_id ."', '" . $dispatch_team_id ."', '" . $current_location . "', '$new_location', '$miles', '$rate', '$commission','$delivery_time','$pickup_datetime'); ";
