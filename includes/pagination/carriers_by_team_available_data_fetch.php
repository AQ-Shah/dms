<?php
		...
		if (isset ($_GET['no_of_record']) && is_numeric($_GET['no_of_record'])) {
			...
			$no_of_records = no_of_available_carriers_by_team_dispatch($user["team_id"]);
			$total_pages = ceil($no_of_records/$record_per_page);
			$page = (isset ($_GET['page'])) ? (int)$_GET['page'] : 1 ;
			$start = ($page-1) * $record_per_page;
			$record_set = find_available_carriers_by_team_dispatch_form_from($user["team_id"],$start,$record_per_page);
		}
		else { 
