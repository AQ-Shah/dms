<?php 
    ...
    if (isset($form['truck_type'])) {$truck_type = $form["truck_type"];} else {$truck_type = null;}
    if (isset($form['mc_validity']) && !empty($form['mc_validity'])) {$mc_validity = $form["mc_validity"];} else {$mc_validity = null;}
    if (isset($form['d_name'])) {$d_name = $form["d_name"];} else {$d_name = null;}
    if (isset($form['d_number'])) {$d_number = $form["d_number"];} else {$d_number = null;}
    if (isset($form['t_length'])) {$t_length = $form["t_length"];} else {$t_length = null;}
    if (isset($form['t_weight'])) {$t_weight = $form["t_weight"];} else {$t_weight = null;}
    if (isset($form['truck_no'])) {$truck_no = $form["truck_no"];} else {$truck_no = null;}
    if (isset($form['trailer_no'])) {$trailer_no = $form["trailer_no"];} else {$trailer_no = null;}
    ...
    $hazmat = isset($form['hazmat']) ? 1 : 0;
    $twic = isset($form['twic']) ? 1 : 0;
    $sida = isset($form['sida']) ? 1 : 0;
    $atp = isset($form['atp']) ? 1 : 0;
