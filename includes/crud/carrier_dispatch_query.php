<?php 
    ...
    if (isset($_POST['datetime'])) {$delivery_time = mysql_prep($_POST["datetime"]);} else {$delivery_time = "";}
    ...
    if (empty($errors)) { 
        ...
        if ($carrier['status'] == 'dispatched') {
         ...
         $query = "INSERT INTO dispatch_list (carrier_id, dispatcher_id, current_location, new_location, rate, delivery_time) ";
         $query .= "VALUES ('" . $carrier["id"] . "', '" . $dispatcher_id ."', '" . $carrier["current_location"] . "', '$new_location', '$rate','$delivery_time'); ";
