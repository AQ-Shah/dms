<?php
		isset ($userData) ? $userID = $userData['id'] : $userID = $user['id'];
		...
		if (isset ($_GET['no_of_record']) && is_numeric($_GET['no_of_record'])) {
			...
			$no_of_records = no_of_carriers_by_sales_agent($userID);
			$total_pages = ceil($no_of_records/$record_per_page);
			$page = (isset ($_GET['page'])) ? (int)$_GET['page'] : 1 ;
			$start = ($page-1) * $record_per_page;
			$record_set = find_carriers_by_sales_agent_form_from($userID,$start,$record_per_page);
		}
		else { 
